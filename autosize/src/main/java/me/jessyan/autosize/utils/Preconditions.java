...
public final class Preconditions {
    ...
    private static String badElementIndex(int index, int size, String desc) {
        if (index < 0) {
            return format("%s (%d) must not be negative", desc, index);
        } else if (size < 0) {
            throw new IllegalArgumentException("negative size: " + size);
        } else {
            return format("%s (%d) must be less than size (%d)", desc, index, size);
        }
    }
    ...
    private static String badPositionIndex(int index, int size, String desc) {
        if (index < 0) {
            ...
            return format("%s (%d) must not be greater than size (%d)", desc, index, size);
        }
    }
    ...
    private static String badPositionIndexes(int start, int end, int size) {
        return start >= 0 && start <= size
                ? (end >= 0 && end <= size
                        ? format("end index (%d) must not be less than start index (%d)", end,
                                start)
