...
public class BaseApplication extends Application {
    ...
    /**
     * 注意!!! 布局的实时预览在开发阶段是一个很重要的环节, 很多情况下 Android Studio 提供的默认预览设备并不能完全展示我们的设计图
     * 所以我们就需要自己创建模拟设备, 以下链接是给大家的福利, 按照链接中的操作可以让预览效果和设计图完全一致!
     * <a
     * href="https://github.com/JessYanCoding/AndroidAutoSize/blob/master/README-zh.md#preview">dp、pt、in、mm
     * 这四种单位的模拟设备创建方法</a>
     * <p>
     * v0.9.0 以后, AndroidAutoSize 强势升级, 将这个方案做到极致, 现在支持5种单位 (dp、sp、pt、in、mm)。
     * {@link UnitsManager} 可以让使用者随意配置自己想使用的单位类型，其中 dp、sp 这两个是比较常见的单位, 作为 AndroidAutoSize 的主单位,
     * 默认支持；pt、in、mm 这三个是比较少见的单位，只可以选择其中一个, 作为 AndroidAutoSize 的副单位, 与 dp、sp 一起被支持。
     * <p>
    ...
    private void configUnits() {
        // AndroidAutoSize 默认开启对 dp 的支持, 可调用 UnitsManager.setSupportDP(false) 关闭。
        // 主单位 dp 和副单位可同时开启的原因是：对于旧项目中已经使用了 dp 布局的页面的兼容。
        ...
        AutoSizeConfig.getInstance().getUnitsManager().setSupportDp(false)
                ...
                // 所以当项目中同时存在副单位、主单位，且副单位设计图尺寸与主单位的不同时，可通过 UnitsManager#setDesignSize() 方法配置
                // 如果副单位的设计图尺寸与主单位的相同, 则不需调用 UnitsManager#setDesignSize()，框架会自动使用 AndroidManifest
                // 中填写的设计图尺寸
                ...
                // AndroidAutoSize 默认不支持副单位, 调用 UnitsManager#setSupportSubunits() 可选择一个自己心仪的副单位,
                // 并开启对副单位的支持
